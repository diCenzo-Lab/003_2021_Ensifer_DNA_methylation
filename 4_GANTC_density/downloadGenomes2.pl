...
foreach $species (@species) { # for each species of interest
	...
	$ftp .= "/@ftp2[9]_cds_from_genomic.fna.gz";
	...
	@ftp2 = split('/',$ftp); # prepare a variable with the current file name
	system("mv @ftp2[10] Sinorhizobium2/$species"); # move the file to the correct directory and rename
	shift(@ftp); # shift the array to the next FTP link
}

#unzip the files
system("gunzip Sinorhizobium2/*.fna.gz");
