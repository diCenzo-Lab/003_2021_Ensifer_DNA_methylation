...
#Loading CACNNNNNTGCG_250.txt
C.CG_250=read.delim("CACNNNNNTGCG_250.txt", header=TRUE, sep="\t")
C.CG_250$Motif_count = as.numeric(as.character(C.CG_250$Motif_count))
head(C.CG_250)
str(C.CG_250)
...
x = merge(C.CG_250, CarbonData, by.x="Gene_name", by.y="Locus_tag", all=FALSE)
x
...
#making the dfC.CG_250 dataframe
dfC.CG_250= data.frame('Gene_name'=C.CG_250$Gene_name, 'Motif_count'= C.CG_250$Motif_count)
dim(dfC.CG_250) #check number of lines
head(dfC.CG_250)

#remove all the genes that are in the carbon source list
dfC.CG_250<- dfC.CG_250[!(dfC.CG_250$Gene_name %in% dfCarbon$Gene_name),]
dim(dfC.CG_250) #checking- works - then continue
...
#naming the column
colnames(dfC.CG_250)=c("Gene_name","Motif_count", "Motif_Promoter")
head(dfC.CG_250)
dfC.CG_250 #the final
...
n.motif=c(dfCarbon$Motif_count,dfC.CG_250$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
G.n.motif=c(dfGlucose$Motif_count,dfC.CG_250$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
S.n.motif=c(dfSucc$Motif_count,dfC.CG_250$Motif_count) #need to line them up
