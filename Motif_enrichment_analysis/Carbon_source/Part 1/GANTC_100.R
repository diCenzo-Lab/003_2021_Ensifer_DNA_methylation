...
#Loading GANTC_100
GANTC_100=read.delim("GANTC_100.txt", header=TRUE, sep="\t")
GANTC_100$Motif_count= as.numeric(as.character(GANTC_100$Motif_count))
head(GANTC_100)
str(GANTC_100)
...
x = merge(GANTC_100, CarbonData, by.x="Gene_name", by.y="Locus_tag", all=FALSE)
x
...
#making the GANTC_100 dataframe
dfGANTC_100= data.frame('Gene_name'=GANTC_100$Gene_name, 'Motif_count'= GANTC_100$Motif_count)
dim(dfGANTC_100) #check number of lines
head(dfGANTC_100)
str(dfGANTC_100)
#remove all the genes that are in the carbon source list
dfGANTC_100<- dfGANTC_100[!(dfGANTC_100$Gene_name %in% dfCarbon$Gene_name),]
dim(dfGANTC_100) #check- yay works thank the lord
#allocating 1 and 0 to the third coloumn, indicating the presence of motif in promoter region
r=1
...
for (i in 1:nrow(dfGANTC_100)){
  
  if (dfGANTC_100[r,2]>0) {
    dfGANTC_100[r,3] = 1
    M.G=M.G+1
  } else {
    dfGANTC_100[r,3] = 0
...
nM.G
#naming the column
colnames(dfGANTC_100)=c("Gene_name","Motif_count", "Motif_Promoter")
head(dfGANTC_100)
dfGANTC_100 #the final dfGANTC_100

#Fisher's exact test
#compare: #GANTC w motif, #GANTC w/out motif, #carbon w motif, #carbon w/out motif
Motif=c(M.C, M.G)
...
n.motif=c(dfCarbon$Motif_count,dfGANTC_100$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
G.n.motif=c(dfGlucose$Motif_count,dfGANTC_100$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
S.n.motif=c(dfSucc$Motif_count,dfGANTC_100$Motif_count) #need to line them up
