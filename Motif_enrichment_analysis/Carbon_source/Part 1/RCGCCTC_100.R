...
#Loading RCGCCTC_100
R.TC_100=read.delim("RCGCCTC_100.txt", header=TRUE, sep="\t") #naming = start and end of motif cuz it s too long
R.TC_100$Motif_count= as.numeric(as.character(R.TC_100$Motif_count))
head(R.TC_100) #check to see
str(R.TC_100)
...
x = merge(R.TC_100, CarbonData, by.x="Gene_name", by.y="Locus_tag", all=FALSE)
x
...
#making the R.TC_100 dataframe
dfR.TC_100= data.frame('Gene_name'=R.TC_100$Gene_name, 'Motif_count'= R.TC_100$Motif_count)
dim(dfR.TC_100) #check number of lines
head(dfR.TC_100)

#remove all the genes that are in the carbon source list
dfR.TC_100<- dfR.TC_100[!(dfR.TC_100$Gene_name %in% dfCarbon$Gene_name),]
dim(dfR.TC_100) #checking- works
...
nM.G 
#naming the column
colnames(dfR.TC_100)=c("Gene_name","Motif_count", "Motif_Promoter")
head(ddfR.TC_100)
dfdfR.TC_100 #the final dfR.TC_100
...
n.motif=c(dfCarbon$Motif_count,dfR.TC_100$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
G.n.motif=c(dfGlucose$Motif_count,dfR.TC_100$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
S.n.motif=c(dfSucc$Motif_count,dfR.TC_100$Motif_count) #need to line them up
