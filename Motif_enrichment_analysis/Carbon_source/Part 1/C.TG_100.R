...
#Loading CGCANNNNNGTG_100.txt
C.TG_100=read.delim("CGCANNNNNGTG_100.txt", header=TRUE, sep="\t")
C.TG_100$Motif_count = as.numeric(as.character(C.TG_100$Motif_count))
head(C.TG_100)
str(C.TG_100)
...
x = merge(C.TG_100, CarbonData, by.x="Gene_name", by.y="Locus_tag", all=FALSE)
x
...
#making the dfC.TG_100 dataframe
dfC.TG_100= data.frame('Gene_name'=C.TG_100$Gene_name, 'Motif_count'= C.TG_100$Motif_count)
dim(dfC.TG_100) #check number of lines
head(dfC.TG_100)

#remove all the genes that are in the carbon source list
dfC.TG_100<- dfC.TG_100[!(dfC.TG_100$Gene_name %in% dfCarbon$Gene_name),]
dim(dfC.TG_100) #checking- works - then continue
...
#naming the column
colnames(dfC.TG_100)=c("Gene_name","Motif_count", "Motif_Promoter")
head(dfC.TG_100)
dfC.TG_100 #the final
...
n.motif=c(dfCarbon$Motif_count,dfC.TG_100$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
G.n.motif=c(dfGlucose$Motif_count,dfC.TG_100$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
S.n.motif=c(dfSucc$Motif_count,dfC.TG_100$Motif_count) #need to line them up
