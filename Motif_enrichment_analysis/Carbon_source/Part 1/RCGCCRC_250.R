...
#Loading RCGCCTC_250
R.TC_250=read.delim("RCGCCTC_250.txt", header=TRUE, sep="\t") #naming = start and end of motif
R.TC_250$Motif_count= as.numeric(as.character(R.TC_250$Motif_count))
head(R.TC_250) #check to see
str(R.TC_250)
...
x = merge(R.TC_250, CarbonData, by.x="Gene_name", by.y="Locus_tag", all=FALSE)
x
...
#making the R.TC_250 dataframe
dfR.TC_250= data.frame('Gene_name'=R.TC_250$Gene_name, 'Motif_count'= R.TC_250$Motif_count)
dim(dfR.TC_250) #check number of lines
head(dfR.TC_250)

#remove all the genes that are in the carbon source list
dfR.TC_250<- dfR.TC_250[!(dfR.TC_250$Gene_name %in% dfCarbon$Gene_name),]
dim(dfR.TC_250) #checking- works - then continue
...
#naming the column
colnames(dfR.TC_250)=c("Gene_name","Motif_count", "Motif_Promoter")
head(dfR.TC_250)
dfR.TC_250 #the final dfR.TC_250
...
n.motif=c(dfCarbon$Motif_count,dfR.TC_250$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
G.n.motif=c(dfGlucose$Motif_count,dfR.TC_250$Motif_count) #need to line them up
...
#T-test 
#gether and arrange the data
S.n.motif=c(dfSucc$Motif_count,dfR.TC_250$Motif_count) #need to line them up
