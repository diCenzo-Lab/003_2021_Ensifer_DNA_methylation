...
genome <- read.delim("CGCANNNNNGTG_100.txt", sep = "\t", header = TRUE) #one of the eight motifs
...
i = 0
for(n in 1:nrow(sym)) {    #runs for as many rows there are
  pos1 <- match(sym[n,1], operon_all[,1])  #matching to any row in operon list column 1
  pos2 <- match(sym[n,1], operon_first[,1]) # all the first operons
  if(is.na(pos1)) { #if it is NA = not in position 1 (ie any operon)
    i <- i + 1
    sym_new[i,1] <- levels(sym$Locus_tag)[sym[n,1]] #put this gene in the new dataframe
  }
...
for(n in 1:nrow(genome)) {    #run for as many rows there are
  pos1 <- match(genome[n,1], operon_all[,1])  #any operon
  pos2 <- match(genome[n,1], operon_first[,1]) #first gene in operons
  if(is.na(pos1)) { #if it is NA = not in position 1 (ie any operon)
    I <- I + 1
