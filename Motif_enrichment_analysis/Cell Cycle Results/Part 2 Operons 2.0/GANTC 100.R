setwd("/Users/janischeng/Desktop/Methylation Project/Analysis")#Load the files
...
#the 8 that are not matched:
unmatched <- cell_new[!(cell_new$Locus_tag %in% x$Gene_name),]
unmatched #just checking
...
y = merge(genome_new, genome, by.x="Locus_tag",by.y ="Gene_name", all=FALSE) #83 genes less, check to see if it s ok
...
#XXunmatched <- cell_new[!(cell_new$Locus_tag %in% XX$Locus_tag),] just checking for unmatched ones but should be ok
